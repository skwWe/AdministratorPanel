...
BASE_LOG_DIR="./logs/sync/$TIMESTAMP"
MAX_JOBS=4
...
for server in "${SERVERS[@]}"; do
    if setup_key_for_server "$server"; then
        READY_SERVERS+=("$server")
    else
        echo "⚠️ Пропускаю $server на этапе сбора: SSH-ключ не настроен"
        server_dir="$BASE_LOG_DIR/$server"
        mkdir -p "$server_dir"
        echo "SSH_ERROR" > "$server_dir/_status.txt"
        echo "SSH-ключ не настроен, сервер пропущен." > "$server_dir/SSH_ERROR.txt"
    fi
done

if [ ${#READY_SERVERS[@]} -eq 0 ]; then
    echo
    echo "❌ Нет серверов с настроенным SSH-доступом. Сбор логов остановлен."
    summarize_results
    exit 3
fi
